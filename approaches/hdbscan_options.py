    ...
    for res, ids in results.items():
        if res == -1:
            ...
            for id in unlabel2label_dist_0_id:
                normals.append(parseInstance(instances[id].src_events, id, 'Normal'))
            ...
            for id, dist in unlabel2mindist.items():
                abnormals.append(parseInstance(instances[id].src_events, id, 'Anomaly'))
                if instances[id].type == 'Normal':
                    FP += 1
                    FP_1 += 1
                else:
...
def option_1(predicts, outlier_scores, instances, labelledNum, logger):
    ...
    for res, ids in results.items():
        if res == -1:
            ...
            TP_1 = 0
            for id in unlabel2label_dist_0_id:
                normals.append(parseInstance(instances[id].src_events, instances[id].src_blk, 'Normal'))
                num_outlier_0 += 1
                normalOutlierCounter[10] += 1
                if instances[id].type == 'Normal':
                    TN += 1
            ...
            for id, dist in unlabel2mindist.items():
                abnormals.append(parseInstance(instances[id].src_events, instances[id].src_blk, 'Anomaly'))
                num_outlier_0 += 1
            ...
            for id in ids:
                ...
                abnormals.append(parseInstance(instances[id].src_events, instances[id].src_blk, 'Anomaly', confidence))
                if instances[id].type == 'Normal':
                    FP += 1
                else:
                    TP += 1
        ...
        # Use a threshold to decide whether those instances classified in normal cores are anomaly or not.
            ...
            for id in ids:
                if id >= labelledNum:
                    ...
                    normals.append(parseInstance(instances[id].src_events, instances[id].src_blk, 'Normal', confidence))
                    if instances[id].type == 'Normal':
                        TN += 1
                    else:
                        FN += 1
